...
class Normalizer(object):
    ...
    def uint16_norm(self):
        ...
        return np.asarray(np.round((self._img-self._min)/(self._max-self._min)*(2**16-1)), dtype=np.uint16)

    def uint8_norm(self):
        ''' normalize image array to 8-bit unsigned integer '''

        return np.asarray(np.round((self._img-self._min)/(self._max-self._min)*(2**8-1)), dtype=np.uint8)
    ...
    def type_norm(self, dtype='float16', lim_min=None, lim_max=None):
        ...
        if dtype.startswith('float'):
            ...
            img_norm = (self._img-self._min)/(self._max-self._min)*(lim_max-lim_min)+lim_min
            ...
            img_norm = np.round((self._img-self._min)/(self._max-self._min)*(lim_max-lim_min)+lim_min)
